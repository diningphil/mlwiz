...
**MLWiz** (*Python for Deep Graph Networks*) is a framework that you can use to easily train and evaluate `Deep Graph Networks <https://www.sciencedirect.com/science/article/abs/pii/S0893608020302197>`_.
...
To this day, we have already used older **MLWiz** versions far beyond the simple examples that are shown here. One can devise `incremental training <https://github.com/diningphil/CGMM>`_ and `continual learning <https://github.com/diningphil/continual_learning_for_graphs>`_  experiments, and we are planning to properly extend the library to
...
For instance, subclassing an :class:`~mlwiz.training.event.handler.EventHandler` requires you to know a bit about how the :class:`~mlwiz.training.engine.TrainingEngine` works and when specific events are triggered.
