...
We provide a general :class:`~mlwiz.data.splitter.Splitter` class that is able to split a dataset of multiple graphs. The most important parameters
...
Here's an snippet of a potential configuration file that downloads and processes the ``NCI1`` graph classification dataset:

.. code-block:: yaml

    dataset:
      root: DATA/
      class_name: mlwiz.data.dataset.NCI1
      args:
        root: DATA/

You can also apply ``transform`` and ``pre_transform`` that follow the same semantic of PyG.

Once our data configuration file is ready, we can create the dataset using (for the example above)

.. code-block:: bash

    mlwiz-dataset --config-file examples/DATA_CONFIGS/config_NCI1.yml

Experiment Setup
**********************

Once we have created a dataset and its data splits, it is time to implement our model and define a suitable task.
Every model must implement the :class:`~mlwiz.model.interface.ModelInterface` interface, and it can optionally use a
readout module that must implement the :class:`~mlwiz.model.interface.ReadoutInterface`.
...
.. code-block:: yaml
    ...
    data_root: DATA
    dataset_class: mlwiz.data.dataset.TUDatasetInterface
    dataset_name:  NCI1
    data_splits_file:  examples/DATA_SPLITS/NCI1/NCI1_outer10_inner1.splits
 ...
 * :class:`~mlwiz.experiment.supervised_task.SupervisedTask` -> ``supervised_config``
 * :class:`~mlwiz.experiment.semi_supervised_task.SemiSupervisedTask` -> ``unsupervised_config`` and ``supervised_config``
...
.. code-block:: bash
    ...
    mlwiz-train --config-file examples/MODEL_CONFIGS/config_SupToyDGN.yml

or

.. code-block:: bash

    mlwiz-train --config-file examples/MODEL_CONFIGS/config_SemiSupToyDGN.yml
...
Assuming you run `mlwiz-train --config-file examples/MODEL_CONFIGS/config_SupToyDGN.yml` inside the MLWiz repo, you can
