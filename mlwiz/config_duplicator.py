...
def main():
    ...
    if len(sys.argv) < 5:
        print("Usage: mlwiz-config-duplicator.py --base-exp-config <base_exp_config> --data-config-files <data_config_files>")
        sys.exit(1)
    ...
    try:
        ...
        for data_config in data_config_files_data:
            ...
            new_config['data_splits_file'] = os.path.join(splits_folder, dataset_name, f"{dataset_name}_outer{outer_folds}_inner{inner_folds}.splits")
